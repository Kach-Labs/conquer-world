...
- Replace **Events** in the top nav with **Gallery**. Events remains a page at `/events` (linked from the homepage upcoming-events panel and from the footer).

## 1. New Gallery page (`/gallery`)

- Short fire-styled hero: "Faith in Action — Moments from the Field".
- Masonry-style responsive grid using the real photos, each with a caption and location/context line.
...
- **Homepage "Who We Are" section**: swap the AI-generated preaching image for the real open-air teaching photo.
...
- Update the site header nav to: Home, About, Programs, Gallery, Devotional, Prayer, Testimonies, Give, Contact.
